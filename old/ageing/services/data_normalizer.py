...
class DataNormalizer:
    ...
    def _get_divisors(self, dataset: Dataset) -> Dict[str, Dict[str, Dict[str, float]]]:
        ...
        Args:
            dataset (Dataset): Dataset to get the divisors from

        Returns:
            Dict[str, Dict[str, Dict[str, float]]]: Dictionary with stored divisors accessed by
                module identifier, channel name, and 'gaussian_divisor' or 'weighted_divisor'
        """
        ...
        for module in dataset.modules:
            ...
            for channel in module.channels:
                ...
                if isinstance(gauss_factor, str):
                    ...
                    divisors[module.identifier][channel.name]["gaussian_divisor"] = gauss_factor
                ...
                if isinstance(weighted_factor, str):
                    ...
                    divisors[module.identifier][channel.name]["weighted_divisor"] = weighted_factor
                logger.debug(f"Calculated divisors for: {channel}")
            logger.debug(f"Calculated divisors for module: {module}")

        logger.info("Sucessfully calculated divisors")
    ...
    def normalize_data(self):
        ...
        for dataset in self.config.datasets:
            for module in dataset.modules:
                for channel in module.channels:
                    channel.set_normalized_ageing_factors(divisors[module.identifier][channel.name])
